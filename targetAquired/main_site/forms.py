...
class SelectCourse(forms.Form):
    allcodes = LabSession.objects.all().values('lab_session')
    choices = []
    for code in allcodes:
        course_code = code['lab_session'].split('-')[0]
        if (course_code, course_code) in choices:
            continue
        choices.append((course_code, course_code))
    course = forms.ChoiceField(choices=tuple(choices))
