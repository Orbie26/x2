...
def makeStudentCards(lab_session, topic_filter=-1):
    ...
    for student in students_waiting:
        ...
        if time_dif > 150: # 2.5 hours limit?
...
def StatisticsView(request, lab_code = -1):
    form = SelectCourse()

    if request.POST:
        form = SelectCourse(request.POST)
        if form.is_valid():
            return HttpResponseRedirect(reverse('stats_filtered', kwargs={'lab_code': int(form.cleaned_data['course'].replace('COMP', ''))}))

    # bar graph comparing number of status types
    fig = plt.figure()
    x_title = 'Question status'
    y_title = 'Quantity'
    title = 'Status of questions at end of session'
    x_series = ['Not started', 'Waiting Question', 'Help needed', 'Gave up', 'Complete', 'Serviced', 'Time out']
    y_series = []
    for bar in x_series:
        if lab_code == -1:
            y_series.append(len(Request.objects.filter(status=bar)))
        else:
            print(Request.objects.filter(lab_session__lab_session__startswith=lab_code))
            y_series.append(len(Request.objects.filter(status=bar, lab_session__lab_session__startswith=(lab_code))))
    ...
    return render(
        request,
        'staff/stats.html', # replace with correct webpage
        {'form': form}
    )
...
def selectLabSessionSTUDENTView(request):
    errorMobileView(request, next_url='select_lab_student')
    if Request.objects.filter(request_location=getLocation(request), status__in=["Not started", "help needed", 'Waiting Question']).exists():
...
def SelectQuestionOrMarkView(request):
    errorMobileView(request, next_url='question_or_mark')
...
def AskQuestionView(request):
    errorMobileView(request, next_url='ask_question')
    form = QuestionForm()
    currentLocation = getLocation(request)
    ...
    return render(
        request,
        'student/ask_question.html', # replace with correct webpage
        {'form' : form}
    )

def WaitView(request):
    errorMobileView(request, next_url='waiting')
    currentLocation = getLocation(request)
    record = tempStoreUserLocationLink.objects.get(location=currentLocation)
    ...
    if Request.objects.filter(request_location=currentLocation, status__in=["Not started", "help needed"]).exists():
        ...
        for query in queryRes:
            ...
            if query.status == "gave up" or query.status == 'cancel' or query.request_end != None:
                quit_count += 1
            if Question.objects.filter(request=query).exists():
                q = Question.objects.get(request=query)
